...
### 5.1. Tabla: `usuarios` — Maestro de Clientes Prontomatic
...
**Cuándo se consulta:** Inmediatamente después de que el webhook de SendGrid entrega un nuevo correo y antes de crear el registro del ticket en PostgreSQL. El campo de búsqueda es el correo electrónico del remitente.

> **Advertencia sobre el esquema real:** Los nombres de tabla y columnas documentados a continuación son los identificados durante la fase de planificación del proyecto. Deben ser **verificados y confirmados** contra el esquema real de la base de datos MySQL de Prontomatic antes de configurar el mapeo en `schema.prisma`. Si los nombres difieren, se deben ajustar en la directiva `@@map` y `@map` del modelo Prisma correspondiente.

| Campo | Tipo de Dato (estimado) | Descripción |
|---|---|---|
| `email` | `VARCHAR` | Dirección de correo electrónico del cliente. Es el campo utilizado para el cruce de datos con el correo del remitente del ticket entrante. Debe existir un índice en este campo para garantizar que la consulta de enriquecimiento sea eficiente. |
| `rut` | `VARCHAR` | RUT del cliente para identificación legal en Chile. Formato esperado por ProntoTicket: string. |
| `direccion` | `VARCHAR` / `TEXT` | Dirección física asociada al cliente o a la comunidad/edificio donde está instalada la maquinaria de Prontomatic. |
| `telefono` | `VARCHAR` | Número de contacto telefónico principal del cliente. Se almacena como string para preservar formatos con guiones, paréntesis o código de país. |
...
// NOTA: El nombre de tabla y columnas debe verificarse contra el esquema real
// de la base de datos MySQL de Prontomatic antes de su uso en producción.
...
model LegacyUsuario {
  ...
  @@map("usuarios")
  @@ignore // Marcar como ignorado en migraciones; esta tabla es external/legacy
}
```
