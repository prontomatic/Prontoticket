...
3. Verificar que el bucket `adjuntos` esté configurado como **privado**.
 
#### 3.1.4. Decisión de Diseño: Row Level Security (RLS) de Supabase
 
Supabase ofrece Row Level Security (RLS) como mecanismo para restringir el acceso a filas específicas de la base de datos directamente desde el cliente (navegador), sin pasar por el servidor. Esta funcionalidad es relevante cuando una aplicación realiza consultas directas a Supabase desde el frontend usando la `anon key`.
 
**En ProntoTicket, las RLS no se configuran ni se requieren.** La razón es la siguiente:
 
Todas las consultas a la base de datos PostgreSQL de ProntoTicket se realizan exclusivamente desde el servidor (API Routes de Next.js) utilizando el `SUPABASE_SERVICE_ROLE_KEY`. Esta clave tiene permisos totales y bypasea las RLS por diseño. El cliente (navegador del agente) nunca se comunica directamente con Supabase — siempre pasa por los API Routes, que se encargan de la autenticación y autorización mediante JWT y RBAC.
 
| Capa | Acceso a Supabase | Clave utilizada | RLS relevante |
|---|---|---|---|
| Servidor (API Routes) | Directo, todas las operaciones | `SUPABASE_SERVICE_ROLE_KEY` | ❌ No aplica (bypaseado) |
| Cliente (navegador) | Nunca accede directamente | — | ❌ No aplica |
 
La seguridad de acceso a los datos está implementada enteramente en la capa de aplicación (API Routes + `authService.js`), que verifica el JWT y los permisos RBAC antes de ejecutar cualquier operación en la base de datos. Esto es arquitectónicamente más robusto que depender de RLS para un sistema de uso interno como ProntoTicket.
 
> **Consecuencia práctica:** Al ejecutar `npx prisma db push`, Supabase creará las tablas sin ninguna política RLS activa. Esto es el comportamiento correcto y esperado para este proyecto. No es necesario configurar políticas adicionales desde el Dashboard de Supabase.
